...
class DataBumil extends Controller
{
    //STOCK DATATABLE
	public function index(Request $request){
		...
		$variabel	= Tbl_ket_variabel::where('tbl_variabel', 'tbl_bumil')->orderBy('ket_variabel', 'ASC')->groupBy('nama_variabel')->get();
		$filter = [];
		...
		Foreach($data as $key=>$val){
			$data[$key] = $this->ket_variabel('tbl_bumil', $val);
	...
	public function view_id($id){
		...
		$result->jumlah_kunjungan           = Tbl_bumil::where('kode_bumil', $result->kode_bumil)->count();
		$result->kunjungan_terakhir         = Tbl_bumil::where('kode_bumil', $result->kode_bumil)->orderBy('id', 'DESC')->first()->tgl_kunjungan;
		$result->pendamping_id         		= $this->attr_pendamping($result->pendamping_id);
		$result->wilayah_id         		= $this->attr_wilayah($result->wilayah_id);
		$result = $this->ket_variabel('tbl_bumil', $result);
	...
	public function view_kode(Request $request, $id){
		$data          = Tbl_bumil::where('kode_bumil', $id)->orderBy('kunjungan', 'ASC')->get();
		Foreach($data as $key=>$val){
			$data[$key] = $this->ket_variabel('tbl_bumil', $val);
	...
	public function report(Request $request){
		...
		$result = Tbl_bumil::where('tgl_kunjungan','>=',$last)->get();
		...
		{
		   $report1['series']['data'][] = Tbl_bumil::where('tgl_kunjungan', $date->format('Y-m-d'))->count();
		...
		$attribut['kunjungan_terakhir'] = Tbl_bumil::orderBy('id', 'DESC')->first()->tgl_kunjungan;
		$attribut['total_kunjungan'] = Tbl_bumil::count();
		$attribut['kunjungan_bulan'] = Tbl_bumil::whereMonth('tgl_kunjungan', $m)->whereYear('tgl_kunjungan', $y)->count();
