...
class DataCatin extends Controller
{
    //STOCK DATATABLE
	public function index(Request $request){
		...
		$data	= $data->where('nik_catin_pria', 'like', '%'.$search.'%');
		if($filter){
		...
		$total  = Tbl_catin::query();
		$total  = $total->where('nik_catin_pria', 'like', '%'.$search.'%');
		...
		Foreach($data as $key=>$val){
			$data[$key] = $this->ket_variabel('tbl_catin', $val);
		}
	...
	public function view_id($id){
		$result          = Tbl_catin::find($id);
		 
		
		$result->jumlah_kunjungan           = Tbl_catin::where('kode_catin', $result->kode_catin)->count();
		$result->kunjungan_terakhir         = Tbl_catin::where('kode_catin', $result->kode_catin)->orderBy('id', 'DESC')->first()->tgl_kunjungan;
		$result->pendamping_id         		= $this->attr_pendamping($result->pendamping_id);
		$result->wilayah_id         		= $this->attr_wilayah($result->wilayah_id);
		$result = $this->ket_variabel('tbl_catin', $result);
	...
	public function view_kode(Request $request, $id){
		$result['data']          = Tbl_catin::where('kode_catin', $id)->orderBy('kunjungan', 'ASC')->get();
	...
	public function report(Request $request){
		...
		$result = Tbl_catin::where('tgl_kunjungan','>=',$last)->get();
		...
		{
		   $report1['series']['data'][] = Tbl_catin::where('tgl_kunjungan', $date->format('Y-m-d'))->count();
		...
		foreach ($kecamatan as $kec)
		{
		   ...
		   $report2['series']['data'][] = Tbl_catin::where('wilayah_id', $kec->kode)->count();
		   $report2['label'][] = $kec->nama;
		} 
		
		$attribut['kunjungan_terakhir'] = Tbl_catin::orderBy('id', 'DESC')->first()->tgl_kunjungan;
		$attribut['total_kunjungan'] = Tbl_catin::count();
		$attribut['kunjungan_bulan'] = Tbl_catin::whereMonth('tgl_kunjungan', $m)->whereYear('tgl_kunjungan', $y)->count();
